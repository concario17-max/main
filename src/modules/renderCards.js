...
const createCardMarkup = ({ slug, href, delayClass, featured, tone, index, portal, icon, heading, copy }) => {
    ...
    >
        ...
        <div class="premium-card-content ${featured ? 'lg:items-start lg:text-left' : ''} justify-center w-full">
            ...
            <div class="premium-card__icon ${toneClasses.icon} ${featured ? 'w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 mb-6 sm:mb-8 lg:mb-10' : 'w-20 h-20 sm:w-24 sm:h-24 mb-6 sm:mb-8'} flex items-center justify-center transition-transform duration-1000 group-hover:scale-110">
                ${icon()}
            </div>
            ...
            <p class="premium-card__description break-keep ${featured ? 'text-slate-800 dark:text-slate-100 text-[14px] sm:text-[15px] md:text-base leading-7 md:leading-8' : 'text-slate-700 dark:text-slate-300 text-[12px] sm:text-[13px] leading-6 sm:leading-relaxed'} font-medium italic mb-6 md:mb-7">
                ${copy.description}
            </p>
