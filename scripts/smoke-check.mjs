...
const main = async () => {
    ...
    */
    ...
    addCheck(indexHtml.includes('A Living Portal for Astrology, Yoga, the Gita, Bardo Wisdom, Atonement, Rosicrucian Esotericism, Samshin Doctrine, and Path Lamp'), 'Header support copy is present');
    ...
    */
    ...
    addCheck(indexHtml.includes('오컬트 주역 천체 관측소, 파탄잘리 요가 수트라, 바가바드 기타, 티베트 사자의 서, 기적수업, 장미십자의 비의, 인위삼신행상명등론, 보리도등론을 아우르는 프리미엄 지식 아카이브.'), 'Meta description is normalized');
    ...
    addCheck(manifestJson.includes('오컬트 주역 천체 관측소, 파탄잘리 요가 수트라, 바가바드 기타, 티베트 사자의 서, 기적수업, 장미십자의 비의, 인위삼신행상명등론, 보리도등론을 아우르는 프리미엄 지식 아카이브.'), 'Manifest description matches current language policy');
